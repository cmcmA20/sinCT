...
**Poset** (or **partially ordered set**) consists of a **carrier set** ``X`` and a binary relation ``\_\leq\_ : X \to X \to Type``, subject to some axioms.

!!! info "Sets and propositions"
	On paper it would be enough to call ``\leq`` a relation to imply that it's valued in propositions.

## Math notation

`` X : Type \dashv ``

`` a \leq b : Type \dashv a , b : X``

``p = q \dashv a, b : X, p, q : a \leq b``

`` a \leq a \dashv a : X ``

`` a \leq c \dashv a, b, c : X, a \leq b, b \leq c ``

``a = b \dashv a, b : X, a \leq b, b \leq a``
