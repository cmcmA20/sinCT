...
# ╔═╡ 805209bd-c363-4cbc-9d7b-441fdc68c952
md"""
# TODO
"""

# ╔═╡ 91640b7e-ee11-41b5-b1a5-d15057d41b83
md"""
## Graphs as C-sets
Graphs are an example of a class of structures called ``C``-sets. This was just illustrated by code and will now be explained mathematically.

``C``-sets are defined relative to a small category ``C``. In this context, the category ``C`` is interpreted as a schema, describing the kinds of parts in the structure and the subpart relations between them. For example, the **schema for graphs** is the category ``Sch(Graph)`` presented by:

``E \overset{src}{\underset{tgt} \rightrightarrows} V``

You may be used to thinking about large categories, such as the category ``Set`` of all sets and functions, but the category ``Sch(Graph)`` is very small! It has exactly two objects, ``E`` and ``V``, and two non-identity morphisms, ``src : E \to V`` and ``tgt : E \to V``. The interpretation is that there are two kinds of parts, of type/sort ``V`` and ``E``. Parts of type ``V`` have no subparts, while parts of type ``E`` have two subparts of type ``V``, called ``src`` and ``tgt``.

Given a small category ``C``, a ``C``-set is a functor ``X : C \to Set``. A ``C``-set ``X`` therefore consists of, for every object ``c : C``, a set ``X(c)``, and for every morphism ``f : c \to d`` in ``C``, a function ``X(f) : X(c) \to X(d)``, subject to the functoriality axioms. In particular, when ``C = Sch(Graph)``, a ``C``-set is exactly a **graph** as previously defined.

!!! info "Synonyms"
	C-sets can be also called **[set diagrams](https://ncatlab.org/nlab/show/diagram)** and **[copresheaves](https://ncatlab.org/nlab/show/copresheaf)**.
"""
...
manifest_format = "2.0"
project_hash = "ade6b817bf7b033b41a21f225e447de1d21de1b9"
...
git-tree-sha1 = "9848ea058c4c7402d0817a41b820158bb3ea7017"
...
git-tree-sha1 = "e31bed0c40b95e4245febdd03023c14f94ef8042"
